...
export async function POST(request: Request) {
  try {
    const user = await requireApiUser();
    const payload = await request.json() as { body?: string; localDate?: string; timezone?: string };
    ...
    const diary = { id: crypto.randomUUID(), title: "", body, status: "saved", localDate: payload.localDate!, createdAt: timestamp };
    await db.batch([
