...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const mention = { id: crypto.randomUUID(), diaryId: id, name, category: payload.category, confidence: 1, evidence: payload.evidence?.trim() || "사용자가 직접 추가한 활동", status: "pending", rawGrowth: 1, createdAt: nowIso() };
    await db.prepare("INSERT INTO activity_mentions (id, diary_id, user_id, name, category, confidence, evidence, status, raw_growth, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(mention.id, id, user.userId, mention.name, mention.category, mention.confidence, mention.evidence, mention.status, mention.rawGrowth, mention.createdAt).run();
