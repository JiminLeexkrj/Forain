...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (diary.status === "confirmed") {
      const existing = await db.prepare("SELECT id, diary_id AS diaryId, name, normalized_name AS normalizedName, category, confidence, evidence, status, raw_growth AS rawGrowth FROM activity_mentions WHERE diary_id = ? AND user_id = ?").bind(id, user.userId).all();
    ...
    for (const mention of mentions) statements.push(db.prepare("INSERT INTO activity_mentions (id, diary_id, user_id, name, normalized_name, category, confidence, evidence, status, raw_growth, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(mention.id, id, user.userId, mention.name, mention.normalizedName || mention.name, mention.category, mention.confidence, mention.evidence, mention.status, mention.rawGrowth, timestamp));
